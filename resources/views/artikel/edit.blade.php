                ...
                <div class="card-body">
                    ...
                    <form action="{{ route('artikel.update',$artikel->id); }}" method="POST" enctype="multipart/form-data">  
                        ...
                        <div class="mb-3">
                          ...
                          <input type="text" name="judul" class="form-control @error('judul') is-invalid @enderror" id="judul" aria-describedby="emailHelp" value="{{ old('judul',$artikel->judul) }}" required>
                          @error('judul')
                          <div class="alert alert-danger mt-4" role="alert">
                            Judul {{ old('judul',$artikel->judul) }} telah dipakai
                          </div>
                          @enderror
                        </div>
                        <label for="foto" class="form-label">Foto</label>
                        <div class="mb-3">
                            <img src="{{ asset("storage/" . $artikel->foto) }}" class="img-thumbnail img-fluid shadow-lg col-sm-5" alt="Artikel Image Edits">
                        ...
                        <div class="input-group mb-3">
                            <select class="form-select fs-5" id="inputGroupSelect02" name="kategori">
                                @foreach ( $kategori as $kat )
                                    @if ( $artikel->kategori != NULL )
                                      <option value="{{ $kat->id }}" 
                                        @if ($kat->nama == $artikel->kategori->nama)
                                         selected 
