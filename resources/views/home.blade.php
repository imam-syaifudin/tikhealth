@extends('layouts.app')

@section('content')
<div class="container">
    <div class="row justify-content-start">
        <div class="col-lg-6 text-center">
            <div class="card shadow-sm">
                <div class="card-header">{{ __('ACTION') }}</div>
                <div class="card-body d-flex justify-content-center">
                        <a href="{{ url('/kategori') }}" class="btn btn-dark mx-1">KELOLA KATEGORI</a>
                        <a href="{{ url('/artikel') }}" class="btn btn-dark">KELOLA ARTIKEL</a>
                        @if ( auth()->user()->role == "admin")
                        <a href="{{ url('/users') }}" class="btn btn-dark mx-1">KELOLA USER</a>
                        <a href="{{ url('/editors') }}" class="btn btn-dark mx-1">KELOLA EDITOR</a>
                        @endif
                </div>
            </div>
        </div>
        <div class="col-lg-6 mt-4">
            <h1 class="fw-bold fst-italic text-center" style="font-size: 4em;"><svg xmlns="http://www.w3.org/2000/svg" width="65" height="65" fill="currentColor" class="bi bi-file-medical-fill" viewBox="0 0 16 16">
                <path d="M12 0H4a2 2 0 0 0-2 2v12a2 2 0 0 0 2 2h8a2 2 0 0 0 2-2V2a2 2 0 0 0-2-2zM8.5 4.5v.634l.549-.317a.5.5 0 1 1 .5.866L9 6l.549.317a.5.5 0 1 1-.5.866L8.5 6.866V7.5a.5.5 0 0 1-1 0v-.634l-.549.317a.5.5 0 1 1-.5-.866L7 6l-.549-.317a.5.5 0 0 1 .5-.866l.549.317V4.5a.5.5 0 1 1 1 0zM5.5 9h5a.5.5 0 0 1 0 1h-5a.5.5 0 0 1 0-1zm0 2h5a.5.5 0 0 1 0 1h-5a.5.5 0 0 1 0-1z"/>
              </svg>TIK HEALTH</h1>
        </div>
        @if( auth()->user()->role == "admin")
        <div class="col-lg-12 mt-4">
            <div class="card shadow-sm">
                <div class="card-header">{{ __('Biodata Diri') }}</div>
                <div class="card-body">
                    @if (session('status'))
                        <div class="alert alert-success" role="alert">
                            {{ session('status') }}
                        </div>
                    @endif
                    <form action="{{ url('/home/cari') }}" method="GET">
                        <div class="input-group mb-3">
                            <select class="form-select" aria-label="Default select example" name="key">
                                <option value="nama">Cari berdasarkan</option>
                                @php
                                    $i = 1;
                                @endphp
                                @foreach( $array as $column )
                                    <option value="{{ $columnName[$i++] }}">{{ strtoupper($column) }}</option>
                                @endforeach
                            </select>
                          <input type="text" class="form-control" placeholder="Cari..." aria-label="Recipient's username" name="value" value="{{ request('value') }}" aria-describedby="button-addon2" autofocus>
                          <button class="btn btn-primary text-light" type="submit">Search</button>
                        </div>
                      </form>
                    <h3 class="text-center fs-1 p-3">@if(isset($key)) Pencarian : {{ strtoupper($soloKey) }}  @else Biodata Diri @endif</h3>
                    {{ $member->links() }} 
                    <div class="table-responsive rounded">
                        <table class="table table-dark text-center shadow-sm">
                            <thead>
                              <tr>
                                <th scope="col">NO</th>
                                <th scope="col">Nama</th>
                                <th scope="col">Tinggi Badan</th>
                                <th scope="col">Berat Badan</th>
                                <th scope="col">BMI</th>
                                <th scope="col">Status Berat Badan</th>
                                <th scope="col">Hobi</th>
                                <th scope="col">Umur</th>
                                <th scope="col">Konsultasi Gratis</th>
                              </tr>
                            </thead>
                            <tbody>
                                @php
                                  $i = 1;
                                @endphp
                                 @foreach ( $member as $mbr)
                                    <tr class="table-light">
                                        <td>{{ $i++ }}</td>
                                        <td>{{ $mbr->nama }}</td>
                                        <td>{{ $mbr->tinggiBadan }}</td>
                                        <td>{{ $mbr->beratBadan }}</td>
                                        <td>{{ $mbr->BMI }}</td>
                                        <td>{{ $mbr->statusBeratBadan }}</td>
                                        <td>@php
                                            echo htmlspecialchars(explode(",",$mbr->hobi)[0]);
                                        @endphp</td>
                                        <td>{{ $mbr->umur }}</td>
                                        @if ( $mbr->konsultasi == 1 )
                                        <td><span class="text-success">Aktif</span></td>
                                        @else
                                        <td><span class="text-danger">Tidak Aktif</span></td>
                                        @endif
                                    </tr>
                                 @endforeach
                            </tbody>
                          </table>
                    </div> 
                </div>
            </div>
        </div>
        @endif
        <div class="col-lg-12">
            <div class="card shadow-sm mt-4">
                <div class="card-header text-center">Introduction</div>
                <div class="card-body">
                    <h1 class="fst-italic fw-bold text-center">Welcome {{ auth()->user()->name }}</h1>
                </div>
            </div>
        </div>
        
    </div>
</div>
@endsection
