...
<div class="container">
    <div class="row justify-content-start">
        <div class="col-lg-4 text-center">
            <div class="card shadow-sm">
                ...
                <div class="card-body d-flex justify-content-around">
                        ...
                        <a href="{{ url('/kategori') }}" class="btn btn-dark">BACK TO KATEGORI</a>
                </div>
            </div>
        </div>
        ...
        <div class="col-lg-12 mt-4">
            <div class="card shadow-sm">
                <div class="card-header">{{ __('Input New Kategori') }}</div>
                <div class="card-body">
                    @if (session('status'))
                    ...
                    @endif
                    <h3 class="text-center fs-1 p-3">Tambah Kategori</h3>
                    <form action="{{ route('kategori.update',$kategori->id) }}" method="POST">  
                        @method('PUT')
                        @csrf
                        <div class="mb-3">
                          <label for="nama" class="form-label">Nama Kategori : </label>
                          <input type="text" name="nama" class="form-control" id="nama" aria-describedby="emailHelp"  required value="{{ old('nama',$kategori->nama) }}">
                          @error('nama')
                          <div class="alert alert-danger mt-4" role="alert">
                            Kategori {{ old('nama') }} telah dipakai
                          </div>
                          @enderror
                        </div>
